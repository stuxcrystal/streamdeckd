...
class EventLoopContext(Context):
    ...
    def on_loop(self, args: Sequence[str], block: Sequence[dict]):
        ...
        asyncio.set_event_loop(factory())

    @validated(min_args=1, max_args=1, with_block=False)
    def on_policy(self, args: Sequence[str], block: Sequence[dict]):
        if len(args) != 1:
            raise ValueError(f"policy: Expected exactly 1 argument, got {len(args)}")
        if block is not None:
            raise ValueError(f"policy: Directive does not accept a block.")
...
class ApplicationContext(Context):
    ...
    def on_eventloop(self, args: Sequence[str], block: Sequence[dict]):
        EventLoopContext().apply_block(block)
    ...
    @validated(min_args=1, max_args=1, with_block=False)
    def on_rescan(self, args: Sequence[str], block: Sequence[dict]):
        self.rescan = parse_timespan(args[0])

    @validated(min_args=1, max_args=1, with_block=False)
    def on_load(self, args: Sequence[str], block: Sequence[dict]):
